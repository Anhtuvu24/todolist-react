...
class TodoList extends React.Component {
    ...
    render() {
        const { list, statusList, keySearch, numberPage } = this.state;
        console.log(this.scrollRef);
        return(
            <div className = "list-container">
                <Header addTodo={this.addTodo} setKeySearch = {this.setKeySearch} displayList = {this.displayList}/>
                <div onScroll={this.onScroll} ref={this.setScrollRef} style={{  overflowY: "scroll", height: 200 }}>
                    {list.map((todo, index) => {
                        let flag = false;
                        if(
                            (
                            ...
                            // (index < (numberPage * 3) && index >= (numberPage * 3 - 3))) 
                        {
                            flag = true;
                            return flag && (
                                <Todo 
